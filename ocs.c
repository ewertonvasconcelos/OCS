...
void
flipflopD (char *nome, int a, int b, int c, int d, char *resetName, double V, double rOut, double cIn, int *clkMuda, double *Vo, int aSet,int bSet, double vSet){
...
netlist[s].j_t0 = (netlist[s].valor / (passo)) * ((e_a - e_b)
- (t0[netlist[s].a] - t0[netlist[s].b]));
}
}
}
}
